...
//  digitalWrite(rst, HIGH);
  ...
  ST7565_st7565_command(CMD_SET_COM_REVERSE);
  ...
  ST7565_st7565_command(CMD_SET_RESISTOR_RATIO | 0x5);
  // initial display line
  // set page address
  // set column address
  // write display data
...
void ST7565_st7565_Write_Buf(uint8_t *c,uint16_t size) {
//  digitalWrite(a0, HIGH);
  //GPIO_SetBits(GPIOB, GPIO_Pin_11);
	LCD_CS_LOW;
	__NOP();
	LCD_RS_HIGH;
	ST7565_spiwrite_buf(c,size);
	LCD_CS_HIGH;
}
...
{
    add=0xb0|add;
    ST7565_st7565_command(add);
	return;
}

void Set_Column_Address(unsigned char add)
{
	ST7565_st7565_command((0x10|(add>>4)));
	ST7565_st7565_command((0x0f&add));
	return;
}
...
void ST7565_refresh(void)
{
	...
	for(uint8_t i=0;i<8;i++)
	{
		...
		Set_Page_Address(i);
		Set_Column_Address(0x00);
		ST7565_st7565_Write_Buf(st7565_buffer+128*i, 128);
//		for(uint8_t j=0;j<128;j++)
//		{
//			ST7565_st7565_data(*);
//			Data++;
